...
void ch32hs_udc_irq(struct usbd_bus *bus)
{
    ...
    if (intflag & USBHS_TRANSFER_FLAG) {
        ...
        memset(g_ch32_usbhs_udc.in_ep, 0, sizeof(struct ch32_usbhs_ep) * CONFIG_CH32HS_BIDIR_ENDPOINTS);
        memset(g_ch32_usbhs_udc.out_ep, 0, sizeof(struct ch32_usbhs_ep) * CONFIG_CH32HS_BIDIR_ENDPOINTS);
